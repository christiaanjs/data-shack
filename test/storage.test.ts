...
describe("POST /api/storage/resolve-write", () => {
  it("returns 401 without auth token", async () => {
    const res = await SELF.fetch("http://localhost/api/storage/resolve-write", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ uris: ["r2://data-shack-storage/output.parquet"] }),
    });
    expect(res.status).toBe(401);
  });
  ...
  it("resolves r2:// write URI to a /api/storage/obj/ URL", async () => {
    const res = await SELF.fetch("http://localhost/api/storage/resolve-write", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({ uris: ["r2://data-shack-storage/output.parquet"] }),
  ...
  it("silently omits http-ds:// URIs (no write support)", async () => {
    const res = await SELF.fetch("http://localhost/api/storage/resolve-write", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({ uris: ["http-ds://cred_123/path"] }),
    });
    expect(res.status).toBe(200);
    const data = (await res.json()) as { urls: Record<string, string> };
...
describe("PUT /api/storage/obj/:token", () => {
  it("returns 401 for an invalid write token", async () => {
    const res = await SELF.fetch("http://localhost/api/storage/obj/invalid-token", {
      method: "PUT",
      body: "data",
    });
  ...
  it("returns 401 when a read token is used for PUT (cross-aud rejection)", async () => {
    const resolveRes = await SELF.fetch("http://localhost/api/storage/resolve", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({ uris: ["r2://data-shack-storage/some-file.parquet"] }),
    });
    const resolveData = (await resolveRes.json()) as { urls: Record<string, string> };
  ...
  it("writes an R2 object and returns 204", async () => {
    const resolveRes = await SELF.fetch("http://localhost/api/storage/resolve-write", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({ uris: ["r2://data-shack-storage/write-test.txt"] }),
    });
    expect(resolveRes.status).toBe(200);
    const resolveData = (await resolveRes.json()) as { urls: Record<string, string> };
...
describe("write token security", () => {
  it("write token key is scoped to the authenticated user", async () => {
    const resolveRes = await SELF.fetch("http://localhost/api/storage/resolve-write", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({ uris: ["r2://data-shack-storage/my-file.parquet"] }),
    });
    const data = (await resolveRes.json()) as { urls: Record<string, string> };
    const writeUrl = data.urls["r2://data-shack-storage/my-file.parquet"] ?? "";

    // Extract token from URL and decode payload (without verifying signature)
    ...
    const payload = JSON.parse(payloadJson) as { k: string; aud: string };
    expect(payload.k).toMatch(/^users\/usr_test\//);
    expect(payload.aud).toBe("storage-write");
  });

  it("r2-s3compat write URI resolves to /api/storage/r2s3compat/obj/ URL", async () => {
    const createRes = await SELF.fetch("http://localhost/api/storage-backends", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({
    ...
    const resolveRes = await SELF.fetch("http://localhost/api/storage/resolve-write", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({ uris: [`r2-s3compat://${id}/output.parquet`] }),
    });
    expect(resolveRes.status).toBe(200);
    const data = (await resolveRes.json()) as { urls: Record<string, string> };
  ...
  it("returns 401 for invalid r2s3compat write token", async () => {
    const res = await SELF.fetch("http://localhost/api/storage/r2s3compat/obj/invalid-token", {
      method: "PUT",
      body: "data",
    });
  ...
  it("returns 401 when a read token is used for r2s3compat PUT (cross-aud rejection)", async () => {
    const createRes = await SELF.fetch("http://localhost/api/storage-backends", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({
        name: "Aud Check Backend",
        type: "r2-s3compat",
        config: {
          endpoint: "https://test-account.r2.cloudflarestorage.com",
          accessKeyId: "k",
    ...
    const resolveRes = await SELF.fetch("http://localhost/api/storage/resolve", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...DEV_HEADERS },
      body: JSON.stringify({ uris: [`r2-s3compat://${id}/file.parquet`] }),
    });
    const resolveData = (await resolveRes.json()) as { urls: Record<string, string> };
