...
```bash
npm test          # 282 vitest tests across 10 test files via @cloudflare/vitest-pool-workers
...
**D1 query layer:** `src/db/queries.ts` for user/identity operations, `src/db/oauth.ts` for OAuth table operations, `src/db/settings.ts` for credentials/storage backends, `src/db/load-jobs.ts` for load job CRUD, `src/db/dashboards.ts` for dashboard CRUD and versioning (`insertDashboard`, `getDashboard`, `listDashboards`, `deleteDashboard`, `updateDashboard`, `snapshotDashboard`, `slugify`, `resolveUniqueSlug`). All SQL is in these files — no inline SQL elsewhere.
...
**MCP server:** `src/mcp/server.ts` — Streamable HTTP transport (MCP 2025-03-26 spec), mounted at `/mcp` on the main Hono app behind `requireAuth`. Twenty-two tools. Core: `get_warehouse_schema` (fetches tables + snapshots from catalog DO; includes a direct-access indicator per table — no browser session required), `list_data_sources` (lists HTTP credentials with their base URLs — no browser session required), `run_query` and `read_data` (both route through the session DO's `/query` endpoint; `read_data` additionally supports `r2://`, `r2-s3compat://`, and `catalog://tableName` URIs for direct streaming without a browser session). Dashboards: `submit_dashboard` (validates and persists a React + Recharts artifact with bound SQL queries to D1; accepts optional `slug` — no browser session required), `list_dashboards` (returns id, slug, title, created_at for all dashboards), `get_dashboard` (retrieves full dashboard detail by id or slug), and `update_dashboard` (updates title/artifact/queries/slug; auto-snapshots the previous version before patching — no browser session required for any of the three). Load jobs: `list_load_jobs`, `create_load_job` (resolves `credential`/`storage_backend` args by name or id via `getCredentialByNameOrId`/`getStorageBackendByNameOrId`, reusing `insertLoadJob`), `update_load_job` (fetches the existing job and merges only the fields present in the call — a true partial patch, unlike the REST `PATCH` handler's implicit full-replace-with-defaults), `delete_load_job`, `set_load_job_enabled` (backed by a new `setLoadJobEnabled` D1 helper in `src/db/load-jobs.ts`; the `enabled` column existed in the schema but had no way to be toggled before this), and `trigger_load_job` — none require a browser session, since load jobs always run server-side. Transform jobs and triggers: `list_transform_jobs`, `create_transform_job`, `update_transform_job`, `delete_transform_job`, `trigger_transform_job`, `list_triggers`, `create_trigger`, `delete_trigger` — these relay to the catalog DO's `/jobs` and `/triggers` endpoints (same validation as the REST routes); only *running* a transform job needs an active browser tab, defining and scheduling it does not.
...
- `frontend/functions/dashboards/[slug]/index.ts` — intercepts all `/dashboards/:slug` requests; fetches the root `index.html` from `ASSETS` (fetching `/` not `/index.html` to avoid an internal Pages redirect), replaces the `<title>` with a slug-derived title (UUID slugs get "Dashboard", real slugs get title-cased), and rewrites the manifest `href` to point to the per-slug manifest endpoint.
- `frontend/functions/dashboards/[slug]/manifest.webmanifest.ts` — returns a per-dashboard Web App Manifest with `name`/`short_name` set to the slug-derived title and `start_url: /dashboards/:slug`. `scope: "/"` is kept so auth and storage are shared with the main PWA context (dashboard shortcuts and the main app operate in one PWA scope).

**PWA assets** (`frontend/public/`): `manifest.webmanifest` (main app manifest with `start_url: /`), `icon-192.png` and `icon-512.png` (solid #1d4ed8 blue squares). `frontend/index.html` links the manifest and sets `<meta name="theme-color">`. Together these enable "Add to Home Screen" for the main app and per-dashboard shortcuts on Android/iOS.
...
- `test/mcp-jobs.test.ts` — load job and transform job MCP tools: create/list/update/set_load_job_enabled/trigger/delete round-trips; credential/storage-backend name-or-id resolution and not-found errors; create/list/update/trigger/delete round-trips for transform jobs; create/list/delete for triggers; partial-update validation (at least one field required)
