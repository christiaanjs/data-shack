...
**MCP server:** `src/mcp/server.ts` — Streamable HTTP transport (MCP 2025-03-26 spec), mounted at `/mcp` on the main Hono app behind `requireAuth`. Eight tools: `get_warehouse_schema` (fetches tables + snapshots from catalog DO; includes a direct-access indicator per table — no browser session required), `list_data_sources` (lists HTTP credentials with their base URLs — no browser session required), `run_query` and `read_data` (both route through the session DO's `/query` endpoint; `read_data` additionally supports `r2://`, `r2-s3compat://`, and `catalog://tableName` URIs for direct streaming without a browser session), `submit_dashboard` (validates and persists a React + Recharts artifact with bound SQL queries to D1; accepts optional `slug` — no browser session required), `list_dashboards` (returns id, slug, title, created_at for all dashboards), `get_dashboard` (retrieves full dashboard detail by id or slug), and `update_dashboard` (updates title/artifact/queries/slug; auto-snapshots the previous version before patching — no browser session required for any of the three).
